...
class VideoProcessor:
    ...
    async def _create_chunk_ultra_fast(self, task: dict) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(
                ...
                timeout=300.0  # 300 секунд таймаут (5 минут)
            )
    ...
    def _create_chunk_direct_command(self, input_path: str, output_path: str, start_time: int, duration: int):
        ...
        if gpu_available:
            # GPU ускоренная команда (NVIDIA) - ОПТИМИЗИРОВАННАЯ ДЛЯ СКОРОСТИ
            cmd = [
                'ffmpeg',
                '-hwaccel', 'cuda',           # Аппаратное декодирование (ускорение)
                ...
                '-preset', 'p1',              # Самый быстрый пресет
                '-tune', 'ull',               # Ультра-низкая задержка (для скорости)
                '-cq', '28',                  # Качество (ниже = лучше, но медленнее)
