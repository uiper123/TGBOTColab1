    ...
    async def create_clips_parallel(self, video_path: str, clip_duration: int, subtitles: list, start_index: int = 0, config: dict = None, max_parallel: int = None) -> list:
        ...
        try:
            ...
            if max_parallel is None:
                ...
                if gpu_available:
                    max_parallel = min(12, len(clips_to_create))  # Максимум 12 для GPU (больше VRAM)
                    logger.info(f"🚀 GPU режим: автоматически выбрано {max_parallel} параллельных процессов для МАКСИМАЛЬНОГО использования VRAM")
                else:
            ...
            try:
                (
                    ...
                    .output(final_video_scaled, audio, output_path, 
                           ...
                           acodec='aac',
                           preset='p1',            # МАКСИМАЛЬНО БЫСТРЫЙ NVENC пресет (p1=fastest)
                           rc='vbr',               # Variable bitrate для NVENC
                           cq=18,                  # Еще более высокое качество (больше GPU нагрузки)
                           pix_fmt='yuv420p',      # Совместимость
                           gpu=0,                  # Принудительно используем первый GPU
                           **{'b:v': '15M',        # МАКСИМАЛЬНЫЙ битрейт (больше GPU работы)
                              'b:a': '192k',       # Увеличенный битрейт аудио
                              'maxrate': '25M',    # МАКСИМАЛЬНЫЙ максимальный битрейт
                              'bufsize': '50M',    # ОГРОМНЫЙ размер буфера (максимум GPU памяти)
                              'surfaces': '64',    # МАКСИМУМ поверхностей для GPU (максимум VRAM)
                              'delay': '0',        # Минимальная задержка
                              'rc-lookahead': '64', # МАКСИМАЛЬНЫЙ lookahead (максимум GPU вычислений)
                              'multipass': 'fullres', # Двухпроходное кодирование (больше GPU работы)
